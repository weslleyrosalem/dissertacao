...
# Initial Hyperparameters (can be changed later through RandomizedSearchCV)
n_steps = 3
n_features = 1
...
def create_model(learning_rate=0.001, num_layers=1, n_steps=3, d_model=64, num_heads=4, loss='mse', dropout_rate=0.2):
    ...
    for _ in range(num_layers):
        x = encoder_layer(512, d_model, num_heads, dropout_rate)(x)
...
random_search = RandomizedSearchCV(estimator=model, param_distributions=param_grid, n_iter=50, n_jobs=-1, cv=3)
